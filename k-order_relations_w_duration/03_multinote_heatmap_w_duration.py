...
# some settings you may want to change in order fot the script to behave diffently
...
def get_list_of_used_notes( _score ):
    ...
    for i, element in enumerate(_score.recurse(classFilter=('Note', 'Rest'))):
        ...
        if isinstance(element, note.Note):
            ...
            if f"{element.nameWithOctave}_{element.duration.type}" not in used_notes:
                used_notes.add( f"{element.nameWithOctave}_{element.duration.type}" )
        else:
            #print( "this is a Rest")
            if f"Rest_{element.duration.type}" not in used_notes:
                used_notes.add( f"Rest_{element.duration.type}" )
...
def get_midinote_from_noteNameWithOctave_and_duration( noteNameWithOctave_and_duration_String ):
    noteNameWithOctaveString = noteNameWithOctave_and_duration_String.split('_')[0]
...
def make2Dgraph( score, _DEPTH ):
    ...
    for i, element in enumerate(score.recurse(classFilter=('Note', 'Rest'))):
        ...
        if( len(tmp_elements_sequence) == _DEPTH):
            l = [ f"{n.nameWithOctave}_{n.duration.type}" if isinstance(n, note.Note) else f"Rest_{n.duration.type}" for n in tmp_elements_sequence]
            # append duration to every element of the list
            #print( l )
            relation_key = tuple(l)
            #print( relationship_key )

            if relation_key not in relations_dict:
                relations_dict[relation_key]  = 1
            else:
                relations_dict[relation_key] += 1
    ...
    #print("\nSTATE RELATIONSHIPS")
    #print(f"We have {len(relations_dict)} relationships")
    #for key in sorted(relations_dict.keys()):
    #    print( f"{key} - {relations_dict[ key ]}" )
    ...
    y_axis_list = []
    for e in list(relations_dict.keys()):
        first_elements_in_relation = e[:-1]
        #print( first_elements_in_relationship )
        # put on y axis only non redundant elements
        if first_elements_in_relation not in y_axis_list:
            y_axis_list.append( first_elements_in_relation)
    ...
    for relation, count in relations_dict.items():
        start = tuple( [ n for n in relation[:-1]] )
        end = relation[-1]
    ...
    if SHOW_PLOT or SAVE_PLOT:
        ...
        plt.xticks(np.arange(len(x_axis_list)), x_axis_list, rotation=45)
    ...
    if SAVE_YAML:
        save_yaml(relations_dict, _DEPTH )

    # TODO: find a way also to plot dot graph for
    # approximation order greater than 2
    # I've just realized that building a dot graph has meaning only for 2nd order relationship.
    #if _DEPTH >= 2 and SAVE_DOT:
    if _DEPTH == 2 and SAVE_DOT:
        makeDotGraph( relations_dict, _DEPTH )
...
def make1Dgraph( score ):
    ...
    for element in score.recurse(classFilter=('Note', 'Rest') ):
        ...
        key = f"{key}_{element.duration.type}"
    ...
    if SHOW_PLOT or SAVE_PLOT:
        ...
        # Add labels and ticks
        plt.xticks(np.arange(num_notes), notes, rotation=45)
        plt.gca().xaxis.tick_top()
        plt.gca().xaxis.set_label_position('top')
        plt.xlabel('Notes', labelpad=10, loc='right')
        plt.yticks([])
...
def makeDotGraph( dict, depth ):
    ...
    for relation, weight in dict.items():
        #start_state = "_".join(list( relationship )[:-1])
        #start_node, _ = relation
        start = tuple( [ n for n in relation[:-1]] )
        end = relation[-1]
        if start not in total_weight_of_edges:
            total_weight_of_edges[start]  = weight
        else:
            total_weight_of_edges[start] += weight
    ...
    for n in USED_NOTES_LIST:
        if 'Rest' in n:
            dot.node(f"{n}", label=n.replace('_','\n'), fillcolor=REST_COLOR, style="filled")
        else:
            nnwo = n.split('_')[0]
            dur  = n.split('_')[1]
            noteNameNoOctave = nnwo[:-1]
            ...
            vertical_position = (get_midinote_from_noteNameWithOctave_and_duration(n) - MIN_MIDINOTE) / MIDINOTE_RANGE
            #print( vertical_position )
            # custom vertical position seems not to work
            dot.node(f"{n}", label=n.replace('_','\n'), pos=f"{0.0},{vertical_position}!", fillcolor=color, style="filled")


    # add  EDGES to the graph
    for relation, weigth in dict.items():
        #start, end = relation
        start = tuple( [ n for n in relation[:-1]] )
        start_name = '_'.join( start )
        end = relation[-1]
        ...
        dot.edge(f"{start_name}", f"{end}", label=str(weight_percentage), fontsize="8", penwidth=str(edge_width))


    # Specify output format (PNG, SVG, etc.)
    output_format = 'png'
    # Save the graph inside the file
    dot.render(f'{OUTPUT_FOLDER}/{MIDIFILENAME[:-4]}_{depth}_order_dotgraph', format=output_format, cleanup=True)
...
if __name__ == '__main__':
    ...
    #midi_file = midi_file.transpose(1)
    ...
    USED_MIDINOTES_LIST = [get_midinote_from_noteNameWithOctave_and_duration(nnwo_dur) for nnwo_dur in USED_NOTES_LIST if nnwo_dur.split('_')[0] != 'Rest' ]

    # USED_MIDINOTES_LIST can contain duplicated (because we can have
    # for example both a 'C4_quarter' and a 'C4_whole' but both have the same
    # midinote). To do this we convert the list in set and back again
    # because sets doesn't allow for duplicates
    USED_MIDINOTES_LIST = list(set(USED_MIDINOTES_LIST))
    ...
    #print( USED_NOTES_LIST )
    #print( USED_MIDINOTES_LIST )
